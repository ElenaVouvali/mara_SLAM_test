...
public:
        ...
        else if (sensor == SensorType::LIVOX)
	{
	    ...
	    for (size_t i = 0; i < tmpLivoxCloudIn->size(); i++)
	    {
	        ...
	        dst.ring = src.line;
	        dst.time = src.time;
//	        RCLCPP_INFO (this->get_logger(), "Lidar timestamp: %f", dst.time);
	    }
	}        
        ...
        timeScanEnd = timeScanCur + laserCloudIn->points.back().time * 1e-9f;
    ...
    bool deskewInfo()
    {
        ...
        if (imuQueue.empty() || ROS_TIME(imuQueue.front().header.stamp) > timeScanCur || ROS_TIME(imuQueue.back().header.stamp) < timeScanEnd)
        {
            ...
            // Log the first and last IMU message timestamps
            RCLCPP_INFO(get_logger(), "First IMU message timestamp: %f", ROS_TIME(imuQueue.front().header.stamp));
    	    RCLCPP_INFO(get_logger(), "Last IMU message timestamp: %f", ROS_TIME(imuQueue.back().header.stamp));

    	    // Log timeScanCur and timeScanEnd
    	    RCLCPP_INFO(get_logger(), "Scan start time (timeScanCur): %f", timeScanCur);
    	    RCLCPP_INFO(get_logger(), "Scan end time (timeScanEnd): %f", timeScanEnd);
